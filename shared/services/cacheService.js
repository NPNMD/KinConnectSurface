...
class CacheService {
    ...
    constructor() {
        if (config_1.config.ENABLE_CACHE && config_1.config.REDIS_URL) {
            this.initialize();
        }
        else {
    ...
    buildKey(namespace, key) {
        return `${config_1.config.CACHE_KEY_VERSION}:${namespace}:${key}`;
    }
    /**
    ...
    async clearVersion() {
        ...
        try {
            const pattern = `${config_1.config.CACHE_KEY_VERSION}:*`;
            const keys = await this.client.keys(pattern);
            if (keys.length > 0) {
                await this.client.del(keys);
                console.log(`Cleared ${keys.length} cache entries for version ${config_1.config.CACHE_KEY_VERSION}`);
            }
        }
        catch (error) {
