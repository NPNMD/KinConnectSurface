...
describe('AccessService', () => {
    ...
    beforeEach(() => {
        ...
        service = new accessService_1.AccessService({ db: mockDb, auditService: mockAuditService });
        jest.clearAllMocks();
    });
    describe('canAccessPatient - Self Access', () => {
        it('should grant access when user accesses their own data', async () => {
            ...
            expect(mockAuditService.logPatientAccess).toHaveBeenCalledWith('user-123', 'user-123', types_1.AuditAction.ACCESS_PATIENT, types_1.AuditResult.SUCCESS, expect.objectContaining({ isSelfAccess: true }));
        });
    });
    describe('canAccessPatient - Family Group Access', () => {
        it('should grant access to users in same family group', async () => {
            ...
            expect(hasAccess).toBe(true);
            expect(mockAuditService.logPatientAccess).toHaveBeenCalledWith('user-123', 'user-456', types_1.AuditAction.ACCESS_PATIENT, types_1.AuditResult.SUCCESS, expect.objectContaining({ reason: 'Family group access' }));
        });
        it('should deny access to users in different family groups', async () => {
            ...
            expect(mockAuditService.logPatientAccessDenied).toHaveBeenCalledWith('user-123', 'user-456', 'User and patient not in same family group');
        });
        it('should deny access when user not found', async () => {
            ...
            expect(hasAccess).toBe(false);
            expect(mockAuditService.logPatientAccessDenied).toHaveBeenCalledWith('nonexistent', 'user-456', 'Requesting user not found');
        });
        it('should deny access when user has no family group', async () => {
            ...
            const hasAccess = await service.canAccessPatient('user-123', 'user-456');
            expect(hasAccess).toBe(false);
            expect(mockAuditService.logPatientAccessDenied).toHaveBeenCalledWith('user-123', 'user-456', 'User not in any family group');
        });
    });
    describe('canAccessPatient - Patient Profile Access', () => {
        ...
        it('should deny access when patient not found', async () => {
            ...
            expect(hasAccess).toBe(false);
            expect(mockAuditService.logPatientAccessDenied).toHaveBeenCalledWith('user-123', 'nonexistent', 'Target patient not found');
        });
    });
    describe('canAccessPatient - Error Handling', () => {
        it('should deny access and log error on exception', async () => {
            ...
            const hasAccess = await service.canAccessPatient('user-123', 'user-456');
            expect(hasAccess).toBe(false);
            expect(mockAuditService.logPatientAccessDenied).toHaveBeenCalledWith('user-123', 'user-456', expect.stringContaining('Database error'));
            expect(console.error).toHaveBeenCalled();
        });
    });
    describe('canAccessPatient - Without Audit Service', () => {
        it('should work without audit service', async () => {
            const serviceWithoutAudit = new accessService_1.AccessService({ db: mockDb });
