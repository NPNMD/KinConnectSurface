...
describe('PatientService', () => {
    ...
    beforeEach(() => {
        ...
        service = new patientService_1.PatientService({ db: mockDb });
        jest.clearAllMocks();
    });
    ...
    describe('getPatientById', () => {
        it('should retrieve a patient by ID', async () => {
            mockDb._mockDoc.get.mockResolvedValue({
                exists: true,
                data: () => testUtils_1.testFixtures.patient
            });
            const result = await service.getPatientById('patient-123');
            expect(result.success).toBe(true);
            expect(result.data).toEqual(testUtils_1.testFixtures.patient);
        });
    ...
    describe('updatePatient', () => {
        it('should update patient successfully', async () => {
            ...
            mockDb._mockDoc.get.mockResolvedValue({
                exists: true,
                data: () => ({ ...testUtils_1.testFixtures.patient, ...updates })
            });
    ...
    describe('getPatients', () => {
        it('should retrieve paginated patients', async () => {
            const patients = [testUtils_1.testFixtures.patient];
            mockDb._mockCollection.get.mockResolvedValue({
    ...
    describe('searchPatientsByCondition', () => {
        it('should search patients by medical condition', async () => {
            mockDb._mockCollection.get.mockResolvedValue({
                docs: [{ data: () => testUtils_1.testFixtures.patient }]
            });
            const result = await service.searchPatientsByCondition('Hypertension');
            expect(mockDb._mockCollection.where).toHaveBeenCalledWith('medicalConditions', 'array-contains', 'Hypertension');
            expect(result.success).toBe(true);
            expect(result.data).toHaveLength(1);
        });
    ...
    describe('searchPatientsByAllergy', () => {
        it('should search patients by allergy', async () => {
            ...
            expect(mockDb._mockCollection.where).toHaveBeenCalledWith('allergies', 'array-contains', 'Penicillin');
            expect(result.success).toBe(true);
        });
    });
