import { RxImageService } from '../rxImageService';
...
describe('RxImageService', () => {
  let service: RxImageService;
  let originalFetch: any;

  beforeEach(() => {
    service = new RxImageService();
  ...
  describe('getImagesByRxcui', () => {
    ...
    it('should fetch images from API and cache', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining('/rximage?rxcui=313782&rLimit=5')
      );
      expect(mockCacheService.set).toHaveBeenCalledWith(
        'image',
        'rxcui:313782',
        apiResponse.nlmRxImages,
        604800
      );
  ...
  describe('getImagesByNdc', () => {
    it('should fetch images by NDC code', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining('/rximage?ndc=12345-678-90')
      );
  ...
  describe('getImagesByName', () => {
    it('should fetch images by drug name', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining('/rximage?name=Aspirin&rLimit=5')
      );
      expect(result).toHaveLength(1);
    });
    ...
    it('should handle special characters in name', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining(encodeURIComponent('Drug & Medicine'))
      );
    });
  });
});
