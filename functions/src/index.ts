...
app.get('/api/drugs/search', authenticate, async (req, res) => {
	try {
		...
		const maxEntries = Math.min(parseInt(limit as string) || 20, 50);
...
app.get('/api/medication-calendar/events', authenticate, async (req, res) => {
	try {
		...
		if (startDate) {
			eventsRef = eventsRef.where('scheduledDateTime', '>=', admin.firestore.Timestamp.fromDate(new Date(startDate as string)));
		}
		if (endDate) {
			eventsRef = eventsRef.where('scheduledDateTime', '<=', admin.firestore.Timestamp.fromDate(new Date(endDate as string)));
		}
		if (status) {
...
app.get('/api/medication-calendar/adherence', authenticate, async (req, res) => {
	try {
		...
		Object.values(medicationAdherence).forEach((med: any) => {
