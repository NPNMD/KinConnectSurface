...
export type MedicationType = 'critical' | 'standard' | 'prn' | 'vitamin';
...
export class GracePeriodEngine {
  ...
  async calculateGracePeriod(
    ...
    const typeRule = patientConfig.medicationTypeRules?.find((rule: any) => rule.medicationType === medicationType);
    if (typeRule) {
   ...
   */
  private async getMedicationOverride(medicationId: string, config: any) {
    if (!config.medicationOverrides || !Array.isArray(config.medicationOverrides)) {
      return null;
    }
    
    return config.medicationOverrides.find((override: any) => 
      override.medicationId === medicationId
    );
  }
   ...
   */
  async getMedicationType(medicationId: string): Promise<MedicationType> {
    try {
   ...
   */
  async getGracePeriodForEvent(event: any): Promise<number> {
    try {
      ...
      if (hour >= 6 && hour < 11) return 30; // morning
      if (hour >= 11 && hour < 17) return 45; // noon/afternoon
      if (hour >= 17 && hour < 21) return 30; // evening
   ...
   */
  async createDefaultGracePeriodConfig(patientId: string): Promise<void> {
    try {
      const defaultConfig = {
        patientId,
   ...
   */
  async updatePatientGraceConfig(patientId: string, updates: any): Promise<void> {
    try {
      const updateData = {
        ...updates,
        patientId,
   ...
   */
  validateGracePeriodConfig(config: any): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
    ...
    if (config.medicationTypeRules && Array.isArray(config.medicationTypeRules)) {
      config.medicationTypeRules.forEach((rule: any, index: number) => {
