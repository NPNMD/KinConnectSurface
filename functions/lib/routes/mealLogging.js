...
exports.registerMealLoggingRoutes = registerMealLoggingRoutes;
 ...
 */
function registerMealLoggingRoutes(app, firestore) {
    // Get meal logs for a patient
    app.get('/meal-logs', middleware_1.authenticate, async (req, res) => {
        try {
    ...
    app.post('/meal-logs', middleware_1.authenticate, async (req, res) => {
        try {
            const patientId = req.user.uid;
    ...
    // Update a meal log
    app.put('/meal-logs/:mealLogId', middleware_1.authenticate, async (req, res) => {
        try {
    ...
    // Delete a meal log
    app.delete('/meal-logs/:mealLogId', middleware_1.authenticate, async (req, res) => {
        try {
            const { mealLogId } = req.params;
            const patientId = req.user.uid;
            const mealLogDoc = await firestore.collection('meal_logs').doc(mealLogId).get();
