...
exports.registerInvitationAndFamilyAccessRoutes = registerInvitationAndFamilyAccessRoutes;
 ...
 */
function registerInvitationAndFamilyAccessRoutes(app, firestore) {
    // ===== INVITATION ROUTES =====
    // Send family invitation
    app.post('/invitations/send', middleware_1.authenticate, async (req, res) => {
        try {
            ...
            try {
                ...
                const rawApiKey = secrets_1.sendgridApiKey.value();
                ...
                const invitationLink = `${constants_1.APP_URL}/invitation/${invitationToken}`;
                    ...
                    html: `
						<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
							...
							<div style="margin-bottom: 20px;">
								<h3 style="color: #1e293b;">Your Access Level: ${constants_1.ACCESS_LEVELS.find(l => l.value === finalAccessLevel)?.label || 'Limited Access'}</h3>
								<p style="color: #64748b; margin-bottom: 15px;">${constants_1.ACCESS_LEVELS.find(l => l.value === finalAccessLevel)?.description || 'You can view and coordinate basic medical information.'}</p>
    ...
    app.post('/invitations/:invitationId/resend', middleware_1.authenticate, async (req, res) => {
        try {
    ...
    // Accept invitation
    app.post('/invitations/accept/:token', middleware_1.authenticate, async (req, res) => {
        try {
            const { token } = req.params;
            const userId = req.user.uid;
    ...
    app.get('/family-access', middleware_1.authenticate, async (req, res) => {
        try {
    ...
    app.get('/family-access/patient/:patientId', middleware_1.authenticate, async (req, res) => {
        try {
    ...
    app.post('/family-access-health-check', middleware_1.authenticate, async (req, res) => {
        try {
            const userId = req.user.uid;
            const userEmail = req.user.email;
    ...
    app.post('/family-access', middleware_1.authenticate, async (req, res) => {
        try {
    ...
    app.delete('/family-access/:accessId', middleware_1.authenticate, async (req, res) => {
        try {
    ...
    app.post('/repair-family-member-patient-links', middleware_1.authenticate, async (req, res) => {
        try {
            const userId = req.user.uid;
            const userEmail = req.user.email;
