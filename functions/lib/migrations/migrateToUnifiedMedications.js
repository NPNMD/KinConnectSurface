...
exports.migrateAllMedications = migrateAllMedications;
exports.rollbackMedication = rollbackMedication;
exports.migrateMedicationsForPatient = migrateMedicationsForPatient;
exports.validateUnifiedMedication = validateUnifiedMedication;
 ...
 */
async function migrateAllMedications(batchSize = 10, dryRun = false) {
    ...
    try {
        ...
        for (let i = 0; i < medications.length; i += batchSize) {
            ...
            const batchResults = await Promise.all(batch.map(doc => migrateSingleMedication(doc.id, dryRun)));
            // Update progress
 ...
 */
async function migrateSingleMedication(medicationId, dryRun) {
    const result = {
        medicationId,
 ...
 */
async function rollbackMedication(medicationId) {
    try {
 ...
 */
async function migrateMedicationsForPatient(patientId, dryRun = false) {
 ...
 */
function validateUnifiedMedication(doc) {
    const errors = [];
    const warnings = [];
    // Required fields
    if (!doc.id)
