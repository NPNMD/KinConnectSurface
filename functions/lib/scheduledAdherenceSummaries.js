...
exports.scheduledAdherencePatternDetection = exports.scheduledMonthlyAdherenceSummaries = exports.scheduledWeeklyAdherenceSummaries = void 0;
const functions = __importStar(require("firebase-functions/v1"));
const admin = __importStar(require("firebase-admin"));
const FamilyAdherenceNotificationService_1 = require("./services/FamilyAdherenceNotificationService");
/**
 * Scheduled function for weekly adherence summaries
 * Runs every Sunday at 8:00 AM UTC (adjusts for patient timezones)
 */
exports.scheduledWeeklyAdherenceSummaries = functions
    .runWith({
    memory: '512MB',
    ...
    const familyNotificationService = new FamilyAdherenceNotificationService_1.FamilyAdherenceNotificationService();
    const results = {
        success: true,
        processed: 0,
    ...
    try {
        ...
        for (const patientId of patientIds) {
            try {
                ...
                const shouldSendSummary = await checkPatientSummaryPreferences(patientId, 'weekly', firestore);
 ...
 */
exports.scheduledMonthlyAdherenceSummaries = functions
    .runWith({
    memory: '512MB',
    timeoutSeconds: 540, // 9 minutes
})
    ...
    try {
        ...
        for (const patientId of patientIds) {
            try {
                ...
                const shouldSendSummary = await checkPatientSummaryPreferences(patientId, 'monthly', firestore);
                if (!shouldSendSummary) {
 ...
 */
exports.scheduledAdherencePatternDetection = functions
    .runWith({
    memory: '512MB',
    timeoutSeconds: 540, // 9 minutes
})
 ...
 */
async function checkPatientSummaryPreferences(patientId, summaryType, firestore) {
    try {
