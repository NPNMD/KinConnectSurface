...
exports.MedicationMonitoringService = void 0;
const admin = __importStar(require("firebase-admin"));
class MedicationMonitoringService {
    firestore = admin.firestore();
    /**
     * Log monitoring metrics for missed medication detection
     */
    async logDetectionMetrics(metrics) {
        try {
            const metricsData = {
                ...metrics,
     ...
     */
    async logGracePeriodMetrics(patientId, calculationTime, cacheHit, appliedRules) {
        try {
            const metricsData = {
                patientId,
                calculationTime,
     ...
     */
    async createSystemAlert(alertType, severity, message, details) {
        try {
            const alert = {
                alertType,
                severity,
    ...
    async monitorDetectionHealth() {
        ...
        try {
            ...
            if (recentMetrics.empty) {
                ...
                const avgErrorRate = metrics.reduce((sum, m) => sum + (m.errorCount / Math.max(m.eventsProcessed, 1)), 0) / metrics.length;
...
exports.MedicationMonitoringService = MedicationMonitoringService;
