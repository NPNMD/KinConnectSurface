...
exports.FamilyAdherenceNotificationService = void 0;
const admin = __importStar(require("firebase-admin"));
const AdherenceAnalyticsService_1 = require("./unified/AdherenceAnalyticsService");
const MedicationNotificationService_1 = require("./unified/MedicationNotificationService");
class FamilyAdherenceNotificationService {
    firestore;
    adherenceService;
    notificationService;
    ...
    constructor() {
        ...
        this.adherenceService = new AdherenceAnalyticsService_1.AdherenceAnalyticsService();
     ...
     */
    async detectAdherencePatterns(patientId, medicationId) {
        try {
        ...
        try {
            ...
            for (const pattern of criticalPatterns) {
                try {
                    ...
                    const eligibleRecipients = await this.filterRecipientsByPreferences(familyMembers, 'pattern_alert', pattern.severity);
     ...
     */
    async generateWeeklySummary(patientId) {
        try {
     ...
     */
    async generateMonthlySummary(patientId) {
        try {
     ...
     */
    async sendAdherenceSummary(summary) {
        try {
            ...
            const eligibleRecipients = await this.filterRecipientsByPreferences(familyMembers, summary.summaryType === 'weekly' ? 'weekly_summary' : 'monthly_summary', 'info');
            if (eligibleRecipients.length === 0) {
     ...
     */
    async getFamilyNotificationPreferences(patientId, familyMemberId) {
        try {
     ...
     */
    async updateFamilyNotificationPreferences(patientId, familyMemberId, preferences) {
        try {
     ...
     */
    async getFamilyMembersForNotification(patientId) {
        try {
     ...
     */
    async filterRecipientsByPreferences(recipients, notificationType, severity) {
        const filtered = [];
        for (const recipient of recipients) {
            ...
            const prefs = await this.getFamilyNotificationPreferences(recipients[0].userId, // patientId from first recipient
            recipient.userId);
     ...
     */
    getDefaultPreferences(patientId, familyMemberId) {
        return {
            patientId,
            familyMemberId,
     ...
     */
    generatePatternAlertMessage(pattern) {
     ...
     * Mark pattern as notified
     */
    async markPatternNotified(patientId, pattern, familyMemberIds) {
        try {
...
exports.FamilyAdherenceNotificationService = FamilyAdherenceNotificationService;
