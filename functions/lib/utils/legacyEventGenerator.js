...
exports.generateCalendarEventsForSchedule = generateCalendarEventsForSchedule;
const admin = __importStar(require("firebase-admin"));
/**
 * Helper function to generate calendar events for a medication schedule
 * Moved from index.ts to reduce monolith size
 */
async function generateCalendarEventsForSchedule(firestore, scheduleId, scheduleData) {
    try {
        ...
        if (events.length > 0) {
            ...
            if (uniqueEvents.length === 0) {
                console.log('⚠️ All generated events would be duplicates - skipping');
                return;
            }
            // Batch write (limit is 500 per batch)
            const batches = [];
            let currentBatch = firestore.batch();
            let count = 0;
            for (const event of uniqueEvents) {
                const docRef = firestore.collection('medication_calendar_events').doc();
                currentBatch.set(docRef, event);
                count++;
                if (count >= 450) { // Safety margin below 500
            ...
            console.log(`✅ Successfully created ${uniqueEvents.length} calendar events`);
        }
        else {
            console.log('⚠️ No future events generated for this schedule');
        }
    }
    catch (error) {
