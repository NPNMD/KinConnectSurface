...
exports.scheduledMissedDetection = void 0;
 ...
 */
exports.scheduledMissedDetection = functions
    .runWith({
    memory: '512MB',
    ...
    try {
        ...
        for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
            ...
            await processBatch(batch, detectionTime, orchestrator, eventService, notificationService, gracePeriodEngine, firestore, results);
        }
        // Step 3: Log summary
 ...
 */
async function processBatch(events, currentTime, orchestrator, eventService, notificationService, gracePeriodEngine, firestore, results) {
    for (const scheduledEvent of events) {
        try {
            ...
            const gracePeriodCalc = await calculateGracePeriodForEvent(scheduledEvent, gracePeriodEngine, firestore);
            ...
            if (currentTime > gracePeriodCalc.gracePeriodEnd) {
                ...
                const workflowResult = await orchestrator.processMissedMedicationWorkflow(scheduledEvent.commandId, {
 ...
 */
async function calculateGracePeriodForEvent(scheduledEvent, gracePeriodEngine, firestore) {
    try {
 ...
 */
function calculateDefaultGracePeriod(scheduledEvent) {
    const scheduledFor = scheduledEvent.timing.scheduledFor;
    const timeSlot = getTimeSlotForScheduledTime(scheduledFor);
 ...
 */
function getTimeSlotForScheduledTime(scheduledTime) {
 ...
 */
async function getFamilyMembersForNotification(patientId, firestore) {
    try {
 ...
 */
async function getPatientInfo(patientId, firestore) {
    try {
