...
function getFamilyNotificationService() {
    if (!familyNotificationService) {
        familyNotificationService = new FamilyAdherenceNotificationService_1.FamilyAdherenceNotificationService();
    }
...
router.put('/family-adherence-preferences/:patientId/:familyMemberId', async (req, res) => {
    try {
        ...
        const service = getFamilyNotificationService();
        const updateResult = await service.updateFamilyNotificationPreferences(patientId, familyMemberId, updates);
