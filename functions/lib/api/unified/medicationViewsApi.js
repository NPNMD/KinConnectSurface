...
router.get('/today-buckets', async (req, res) => {
    try {
        ...
        const buckets = {
            ...
            dueSoon: [],
            morning: [],
            lunch: [], // Updated from 'noon'
            evening: [],
            beforeBed: [], // Updated from 'bedtime'
            overdue: [],
            completed: [],
            lastUpdated: now
        };
        ...
        for (const command of commands) {
            if (command.status.isPRN)
                continue; // Skip PRN medications
            ...
            const completedEvent = medicationEvents.find(e => e.eventType === unifiedMedicationSchema_1.MEDICATION_EVENT_TYPES.DOSE_TAKEN ||
                e.eventType === unifiedMedicationSchema_1.MEDICATION_EVENT_TYPES.DOSE_MISSED ||
                e.eventType === unifiedMedicationSchema_1.MEDICATION_EVENT_TYPES.DOSE_SKIPPED);
            if (completedEvent) {
            ...
            const scheduledEvent = medicationEvents.find(e => e.eventType === unifiedMedicationSchema_1.MEDICATION_EVENT_TYPES.DOSE_SCHEDULED);
                    ...
                    switch (command.preferences.timeSlot) {
                        ...
                        default:
                            ...
                            if (hour >= 6 && hour < 11)
                                ...
                                buckets.lunch.push(bucketItem); // Updated from 'noon'
                            else if (hour >= 17 && hour < 21)
                                buckets.evening.push(bucketItem);
                            else
                                buckets.beforeBed.push(bucketItem); // Updated from 'bedtime'
                    }
                }
            }
        }
...
router.get('/dashboard', async (req, res) => {
    try {
        ...
        const adherenceResult = await getEventService().calculateAdherenceMetrics(targetPatientId, thirtyDaysAgo, new Date());
...
router.get('/calendar', async (req, res) => {
    try {
        ...
        const { patientId, startDate, endDate, view = 'week' } = req.query;
        const targetPatientId = patientId || userId;
        // Check access permissions
        if (targetPatientId !== userId) {
            // TODO: Add family access validation here
        ...
        const calendarData = {
            ...
            summary: {
                ...
                scheduledEvents: events.filter(e => e.eventType === unifiedMedicationSchema_1.MEDICATION_EVENT_TYPES.DOSE_SCHEDULED).length,
                takenEvents: events.filter(e => e.eventType === unifiedMedicationSchema_1.MEDICATION_EVENT_TYPES.DOSE_TAKEN).length,
                missedEvents: events.filter(e => e.eventType === unifiedMedicationSchema_1.MEDICATION_EVENT_TYPES.DOSE_MISSED).length,
                skippedEvents: events.filter(e => e.eventType === unifiedMedicationSchema_1.MEDICATION_EVENT_TYPES.DOSE_SKIPPED).length
            },
...
router.get('/dashboard', async (req, res) => {
    try {
        ...
        const dashboardSummary = {
            ...
            todayStatus: {
                now: todayBuckets?.now?.length || 0,
                dueSoon: todayBuckets?.dueSoon?.length || 0,
                overdue: todayBuckets?.overdue?.length || 0,
                completed: todayBuckets?.completed?.length || 0,
                totalScheduled: (todayBuckets?.now?.length || 0) +
                    (todayBuckets?.dueSoon?.length || 0) +
                    (todayBuckets?.morning?.length || 0) +
                    (todayBuckets?.lunch?.length || 0) + // Updated from 'noon'
                    (todayBuckets?.evening?.length || 0) +
                    (todayBuckets?.beforeBed?.length || 0) // Updated from 'bedtime'
            },
            adherence: {
                last30Days: {
                    adherenceRate: adherenceResult.data?.adherenceRate || 0,
