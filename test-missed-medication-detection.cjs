...
async function testMissedMedicationDetection() {
  ...
  try {
    ...
    // Step 5: Test medication type classification
    console.log('\n📋 Step 5: Testing medication type classification...');
...
async function testPatientGraceConfig(patientId) {
  ...
  try {
    ...
    if (configDoc.exists) {
      ...
      console.log('   📝 Would create default config with grace periods:', {
        morning: defaultConfig.defaultGracePeriods.morning,
        noon: defaultConfig.defaultGracePeriods.noon,
        evening: defaultConfig.defaultGracePeriods.evening,
        bedtime: defaultConfig.defaultGracePeriods.bedtime
      });
    }
    ...
    if (prefsDoc.exists) {
      ...
      console.log('   ✅ Found patient medication preferences:', {
        ...
        hasTimeSlots: !!prefs.timeSlots,
        hasGracePeriodSettings: !!prefs.gracePeriodSettings
      });
      
      if (prefs.gracePeriodSettings) {
        console.log('   📊 Grace period settings in preferences:', prefs.gracePeriodSettings);
      }
    } else {
...
async function testMissedDetectionLogic() {
  ...
  try {
    ...
    console.log(`\n📊 Detection Summary:`);
    console.log(`   Events past grace period: ${overdueCount}`);
    console.log(`   Events within grace period: ${withinGraceCount}`);
    console.log(`   Total events analyzed: ${overdueQuery.docs.length}`);
...
async function testMedicationTypeClassification() {
  console.log('🔍 Testing medication type classification...');
...
// Test grace period defaults
async function testGracePeriodDefaults() {
  console.log('\n📋 Testing grace period defaults...');
  ...
  console.log('   📊 Default grace periods by medication type:');
  Object.entries(defaultGracePeriods).forEach(([type, periods]) => {
    console.log(`      ${type}:`, periods);
  });
  
  // Test time slot classification
  const testTimes = [
    { time: '07:00', expectedSlot: 'morning' },
    { time: '12:00', expectedSlot: 'noon' },
    { time: '18:00', expectedSlot: 'evening' },
    { time: '22:00', expectedSlot: 'bedtime' },
    { time: '02:00', expectedSlot: 'bedtime' }
  ];
  
  console.log('\n   📊 Time slot classification tests:');
  testTimes.forEach(({ time, expectedSlot }) => {
    const [hours] = time.split(':').map(Number);
    
    let actualSlot = 'morning';
    if (hours >= 11 && hours < 17) actualSlot = 'noon';
    else if (hours >= 17 && hours < 21) actualSlot = 'evening';
    else if (hours >= 21 || hours < 6) actualSlot = 'bedtime';
    
    const isCorrect = actualSlot === expectedSlot;
    console.log(`      ${isCorrect ? '✅' : '❌'} ${time} → ${actualSlot} (expected: ${expectedSlot})`);
  });
}
...
module.exports = {
  ...
  testMedicationTypeClassification
};