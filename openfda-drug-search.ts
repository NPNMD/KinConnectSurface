// OpenFDA-based drug search implementation
// This replaces the RxNorm-only approach with a hybrid OpenFDA + RxNorm strategy

// Search for drugs by name using OpenFDA API with RxNorm fallback
export async function searchDrugsOpenFDA(query: string, limit: number = 20) {
	const searchLimit = Math.min(limit, 50); // Cap at 50 results
...
export const openFDADrugSearchRoute = `
// Search for drugs by name using OpenFDA API with RxNorm fallback
app.get('/drugs/search', authenticate, async (req, res) => {
	try {
		...
		const drugConcepts = await searchDrugsOpenFDA(query, parseInt(limit as string, 10));
